...
class UserServiceTest {
    ...
    inner class GetOrCreateUser() {
        ...
        inner class NewUser() {
            ...
            fun `should create and return new user`() {
                whenever(userRepository.findByDiscordId(discordUser.id)).thenReturn(Optional.empty())
                whenever(roleRepository.findByName(RoleConstants.USER_ROLE)).thenReturn(Optional.of(userRole))
        ...
        inner class PreExistingUser() {
            ...
            fun `should return existing user`() {
                whenever(userRepository.findByDiscordId(discordUser.id)).thenReturn(Optional.of(expectedUser))
    ...
    inner class GetCurrentUser() {
        ...
        fun `should return the current user`() {
            ...
            val expectedUserResponse = UserResponse(uuid, expectedUser.displayName, expectedUser.timeZoneId)
    ...
    inner class UpdateUser() {
        ...
        inner class PreExistingUser() {
            ...
            fun `should update the user`() {
                val updatedUser = expectedUser.apply {
                    displayName = updateUserRequest.displayName
                    timeZoneId = updateUserRequest.timeZoneId.toInt()
                }

                whenever(userRepository.findByUsername(expectedUser.username)).thenReturn(Optional.of(expectedUser))
        ...
        inner class NonExistingUser() {
            ...
            fun `should throw UserNotFoundException`() {
                whenever(userRepository.findByUsername(expectedUser.username)).thenReturn(Optional.empty())
